...
public class CheckParentNodeFaultTO_QUEUEJob implements Job {
	private static final Logger log = LoggerFactory.getLogger(CheckParentNodeFaultTO_QUEUEJob.class);
	private Map<String,AllFaultData> faultMapTemp = new HashMap<String, AllFaultData>();
	
	public void execute(JobExecutionContext context) throws JobExecutionException {
		Date d = new Date();
		log.info("开始进行父节点故障检测(TO_QUEUE)[{}]",DateUtil.format2Str(d));
		Context.reviceFlag = false;
		checkParentNodeMethod();
		Context.reviceFlag = true;
		log.info("结束进行父节点故障检测(TO_QUEUE)[{}]",DateUtil.format2Str(d));
	}
	//修复逻辑
...
//						ReceiveMsgMQ.getInstance().sendMsgToMQ(svo.toStatusMsgString());
						...
						MqQueueMsgSender.sendMessage(VariableInit.MQ_MAINTAININF_TOPIC, faultMapTemp.get(devicekey).getFault().toStatusMsgString());
