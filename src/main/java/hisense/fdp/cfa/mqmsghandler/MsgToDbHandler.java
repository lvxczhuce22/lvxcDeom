 ...
 * 
 */
public class MsgToDbHandler implements Runnable {
	private static final Logger log = LoggerFactory.getLogger(MsgToDbHandler.class);
	// 任务处理线程无待处理任务时线程等待时间，单位毫秒
	public static final long TIME_WAIT_WORKING = 1000;
	// 接收mq消息的观察者
	private final MQMsgToStorageObserver mQMsgToStorageObserver;
	// 处理消息接口集合
	private final Map<String, IMsgHandlerService> msgHandlerServiceMap;

	public MsgToDbHandler(IObserver mQMsgToStorageObserver, Map<String, IMsgHandlerService> msgHandlerServiceMap) {
		this.mQMsgToStorageObserver = (MQMsgToStorageObserver)mQMsgToStorageObserver;
	...
	public void run() {
		while (true) {
			try {
				while(!Context.reviceToDbFlag){
					log.info("正在结算分析实时数据检测结果，暂停读取消息队列!!!");
					Thread.sleep(5*TIME_WAIT_WORKING);
				}
				if (mQMsgToStorageObserver.getEventQueueLength() == 0) {
					Thread.sleep(TIME_WAIT_WORKING);
				} else {
					// 多线程同时取可能存在判断队列长度时不为0，但取消息时已被其他线程取走，需要再次判空
					ConnectorEvent event = mQMsgToStorageObserver.getConnectorEventByPriority();
					if (null == event) {
						Thread.sleep(TIME_WAIT_WORKING);
					} else {
						...
						msgHandlerImpl.msgDetailParser(event);
