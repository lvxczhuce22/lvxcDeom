...
public class MqQueueMsgSender
{
	private static final Logger log = LoggerFactory.getLogger(MqQueueMsgSender.class);
...
//					"failover:(nio://127.0.0.1:61616,nio://127.0.0.1:61616)?randomize=false");
			connectionFactory = new ActiveMQConnectionFactory(VariableInit.MQ_SEND_USER, VariableInit.MQ_SEND_PWD,
					VariableInit.MQ_SEND_URL);
	...
	public static boolean sendMessage(String destinationTopic, String message)
	{
		...
		try
		{
			...
			Destination queue = session.createQueue(destinationTopic);
